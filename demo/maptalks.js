...
var TileLayerDomRenderer = function (_Class) {
    ...
    TileLayerDomRenderer.prototype._getTileQueue = function _getTileQueue(tileGrid) {
        ...
        if (this._preCenterId) {
            var prePos = this._tiles[this._preCenterId]['viewPoint'];
            var current = void 0;
        ...
        for (var _i = tiles.length - 1; _i >= 0; _i--) {
            ...
            if (offset && !offset.isZero()) {
                tiles[_i]['viewPoint']._sub(offset);
            }
            queue.push(tiles[_i]);
        }
    ...
    TileLayerDomRenderer.prototype._updateContainer = function _updateContainer() {
        ...
        if (!domMat) {
            ...
            if (this._centerOffset && !this._centerOffset.isZero()) {
                style = Browser$1.any3d ? 'translate3d(' + this._centerOffset.x + 'px, ' + this._centerOffset.y + 'px, 0px) ' : 'translate(' + this._centerOffset.x + 'px, ' + this._centerOffset.y + 'px) ';
            }
            if (fraction !== 1) {
        ...
        if (fraction !== 1) {
            ...
            if (map.isZooming() && this._zoomParam) {
                var origin = this._zoomParam['origin'],
                    pitch = map.getPitch(),
                    matOffset = [(origin.x - size['width'] / 2) * (1 - fraction), (origin.y - size['height'] / 2) * (1 - fraction) * (pitch ? Math.cos(pitch * Math.PI / 180) : 1), 0];
        ...
        var tileOffset = mapOffset.multi(1 / fraction);
...
typeof console !== 'undefined' && console.log('maptalks v0.28.0-alpha');
